...
local g_TradeRoute	= UILens.CreateLensLayerHash("TradeRoutes")
-- GCO >>>>>
...
-- GCO <<<<<
local m_uiVillages			:table = {};	-- Potential Villages
-- ===========================================================================
function OnClickPotentialVillage( villagePlotId:number )

	LuaEvents.VillageSelected(villagePlotId)
...
-- ===========================================================================
function ShowPotentialVillagesPlots(plotID)

	local pPlot		= GCO.GetPlotByIndex(plotID)
	local playerID	= Game.GetLocalPlayer()
	if pPlot then

		local bCanShowRoute = not (UI.GetInterfaceMode() == InterfaceModeTypes.CITY_MANAGEMENT or UI.GetInterfaceMode() == InterfaceModeTypes.MAKE_TRADE_ROUTE)
		UILens.SetActive(g_TradeRoute)
		UILens.ClearLayerHexes( g_TradeRoute )

		local pLocalPlayerVis 	= PlayersVisibility[playerID]
		for i, iPlot in ipairs(GCO.GetPlotsInRange(pPlot, 4)) do
			local testPlot	= GCO.GetPlotByIndex(iPlot)
			local roadPath	= pPlot:GetRoadPath(testPlot, "Land", 6)
			if roadPath and GCO.CanPlaceTribe(testPlot, 2, playerID) and GCO.NoAdjacentImprovement(testPlot) and pLocalPlayerVis:IsRevealed(testPlot:GetX(), testPlot:GetY()) then
				local pInstance:table = GetInstanceAt( iPlot );
				if pInstance ~= nil then
					table.insert( m_uiVillages, pInstance );
					pInstance.PlaceVillageButton:SetVoid1( iPlot );
					pInstance.PlaceVillageButton:RegisterCallback(Mouse.eLClick, OnClickPotentialVillage );
					pInstance.PlaceVillageButton:SetHide(false);
					pInstance.PlaceVillageButton:SetDisabled( false );
					
					pInstance.PlaceVillageLabel:SetText( Locale.Lookup("LOC_PLOTINFO_PLACE_VILLAGE", testPlot:GetX(), testPlot:GetY()) );
					--LOC_PLOTINFO_PLACE_VILLAGE
					-- get resources for tooltip
					local resourceValues = testPlot:GetBaseVisibleResources(playerID)
					for ii, adjacentPlotID in ipairs(GCO.GetAdjacentPlots(testPlot)) do
						local adjacentPlot = GCO.GetPlotByIndex(adjacentPlotID)
						if adjacentPlot and pLocalPlayerVis:IsRevealed(adjacentPlot:GetX(), adjacentPlot:GetY()) then
							for resourceID, value in pairs(adjacentPlot:GetBaseVisibleResources(playerID)) do
					...
					for resourceID, value in pairs(resourceValues) do
						table.insert(tStrTooltip,  GCO.GetResourceIcon(resourceID).. " " .. Indentation(Locale.Lookup(GameInfo.Resources[resourceID].Name), 10) .. Indentation(value, 3, true))
					end
					--local sToolTip = Locale.Lookup("LOC_PLOTINFO_PLACE_VILLAGE_TOOLTIP") .. "[NEWLINE][NEWLINE]" .. table.concat(tStrTooltip, "[NEWLINE]")
					
					local population	= testPlot:GetPopulation()
					local popVariation 	= population - testPlot:GetPreviousPopulation() 
					...
					kParameters.Title2	= Locale.Lookup("LOC_TOOLTIP_SEPARATOR_SMALL")
					kParameters.Title3	= Locale.Lookup("LOC_PLOTINFO_PLACE_VILLAGE_TOOLTIP") -- LOC_CITYBANNER_SMALL_POPULATION_TITLE
					kParameters.List3	= table.concat(tStrTooltip, "[NEWLINE]")
					
					GCO.AttachCustomToolTip(pInstance.PlaceVillageLabel, kParameters)
					
					if bCanShowRoute then
					local kVariations 	= {}
						local color 		= UI.GetColorValue(0.90, 0.90, 0.90, alphaRatio) --RGBAValuesToABGRHex(1, 1, 1, 1) --RGBAValuesToABGRHex
						UILens.SetLayerHexesPath( g_TradeRoute, Game.GetLocalPlayer(), roadPath, kVariations, color )
					end
					
					local nameSize = pInstance.PlaceVillageLabel:GetSizeX() + 4
					pInstance.PlaceVillageButton:SetSizeX(nameSize)
				end
			end
		end
	end
end
LuaEvents.SelectVillageClicked.Add(ShowPotentialVillagesPlots)

-- ===========================================================================
function HidePotentialVillagesPlots()
	
	
	for _,kInstance in ipairs(m_uiVillages) do
		kInstance.PlaceVillageButton:SetHide( true );
	end
	m_uiVillages = {};
...
LuaEvents.HideVillagePlots.Add(HidePotentialVillagesPlots)
-- GCO >>>>>
