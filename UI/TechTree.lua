...
local PADDING_TIMELINE_LEFT			:number = 175;
	...
	for row:table in GameInfo.Eras() do
		for era,eraData in pairs(m_kEras) do
			if eraData.Index == index then									-- Ensure indexed order
				...
				priorColumns = priorColumns + eraData.NumColumns --+ 1;	-- 
				-- GCO >>>>>
...
STATUS_ART[ITEM_STATUS.RESEARCHED]	= { Name="RESEARCHED",	TextColor0=0xaaffffff, TextColor1=0x88000000, FillTexture="TechTree_GearButtonTile_Done.dds",	BGU=0,BGV=(SIZE_NODE_Y*5),	IsButton=false,	BoltOn=true,	IconBacking=PIC_METER_BACK_DONE  };
		...
		function CleanPrereq()
			for i,itemType in pairs(item.Prereqs) do
				if itemType ~= "_TREESTART" then
					...
					otherNode.NodeButton:SetAlpha(1)
					otherNode.OtherStates:SetAlpha(1)
		...
		node.NodeButton:RegisterCallback(		Mouse.eMouseEnter,	ShowPrereq);--RegisterMouseOverCallback(ShowPrereq(item))
		node.NodeButton:RegisterCallback(		Mouse.eMouseExit,	CleanPrereq);--RegisterMouseOverCallback(ShowPrereq(item))
