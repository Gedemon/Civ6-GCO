...
table
[CivID1]
	[CivID2] = { Trust = { [ModifierID] = value, ...}, Relation  = { [ModifierID] = value, ...}}
	[CivID3] = ...
...
diplomacy level
	units : discuss with the commander of one unit (barbarian or civ/tribe unit with low loyalty)
		- recruit as mercenary (limited time, became barbarian with x turn of truce at the end of contract)
		- renew contract
		- bribe for individual truce for x turn
	tribe : "fake" playeID for tribes = (cultureGroupIndex + 100)
		- ally
			- mobilize and move to: mobilize units and move to a point, attacking enemy on the way
			- mobilize and rush to: mobilize units and rush to a point, ignoring enemy on the way 
	civilization
	
tributes
	create new unit type to move tribute on map and can be captured ?
	
diplomacy currency
	gold
	resources (if trade route/supply route)
	slave (if trade route/supply route)
	
diplomacy action require envoys
	each active deal requires x available envoys
	can't do new deal if not enough available envoy left
	ending a deal give back the envoys
	multiple-civ deals require x envoys per civ
...
type of deal (immediate effect on validation)
	...
	borders map
	denounce
	give territory in which a civ has a majority of culture
	remove embargo
	stop declaration of friendship
	stop defensive pact
	stop open borders
	stop open market
...
+ gain experience from
	+ combat (combart type / terrain / units tag)
	+ movement (terrain type / feature type)
- loss experience from
	- reinformcement (mitigated based on buildings in linked city)
	
Automated promotions based on experience
--]]
...
local
	lower
		...
		lack of food or other needs
		lack of justice (absence of related buildings) / high criminality
		high taxes
		low health
...
Construction            (Masonry, The Wheel)
...
goal is to create a settler or city:
	- no standing army
		all units have a 5 turns timer
		on disband send pop/res/equipment to central village
		suspend timer on migration
	- minimal requirement
		population 3500
		materiel	350 ?
		x% of own Culture group or 1750 pop of own culture group
	- options
		abandon area (count all resources and population from all settlements -> faster) -> "migration" mode
		expand settlement into town (available once requirement reached in central settlement, convert to city, keep the satellite settlements)
		create settler (keep all settlements, create settler, convert settlements to ally tribes when creating first city (or limited player action to act as ally), and keep units close to the city for the player) -> longer, require additionnal population as some stay in the settlement
	- research (only 1 possible)
		farming 			-> +x point per farm resource collected	-> create farm improvement on adjacent resources
		sailing 			-> +x point/turn by coastal villages	-> create fishing boat on adjacent resources
		bronze working 		-> +x point per Copper used				-> can create bronze armor/swords/spears
		horseback riding 	-> +x points per trained horse			-> can train war horses
		
population include slaves (captured in other settlements)

slaves can be converted into "slave worker unit" that passively collect resources

materiel created each turn from
	wood
	stone
	clay
	
luxuries converted to gold each turn (or helping assimilation ?)

additional buildings on city creation with enough materiel/resources (could be carried by settler)
	- Carpenter
		wood		200
		materiel
...
pillaged satellite settlements respawn after a few turn (or cotrelloed by central settlement ?)

captured central settlement can either be captured with its satellites (with a gold cost for repair) or pillaged (with loot from all satellites ? or make them independent ?), with a new tribe of multiple settlements being spawned at another position if possible

pillaged independent settlements can be captured if close enough of a center settlement, or diseapear and a new settlment is placed if a plot is found at another position on the map.
...
+ Victory Points
	- Choice of medium-term objective with possible multi-goal path
	- Accomplish all goals to get max points
	- Spy can reveal other players objective/intermediate goals
	
+ Objectives can be announced to gain more points
