...
INSERT OR REPLACE INTO Civilizations (CivilizationType, Name, Description, Adjective, StartingCivilizationLevelType, RandomCityNameDepth, Ethnicity, StartingDate, StartingEra)
	SELECT	'CIVILIZATION_' || Name, 'LOC_CIVILIZATION_' || Name || '_NAME', 'LOC_CIVILIZATION_' || Name || '_DESCRIPTION', 'LOC_CIVILIZATION_' || Name || '_ADJECTIVE', 'CIVILIZATION_LEVEL_FULL_CIV', 10, 'ETHNICITY_' || Ethnicity, StartingDate, StartingEra
	FROM CivilizationConfiguration;
	
UPDATE Civilizations SET StartingCivilizationLevelType='CIVILIZATION_LEVEL_TRIBE'
	WHERE EXISTS	(SELECT * FROM CivilizationConfiguration WHERE Civilizations.CivilizationType = 'CIVILIZATION_' || CivilizationConfiguration.Name AND CivilizationConfiguration.IsTribe = 1);
	
UPDATE Civilizations SET StartingEra= 'ERA_' || StartingEra	WHERE StartingEra NOT NULL;
